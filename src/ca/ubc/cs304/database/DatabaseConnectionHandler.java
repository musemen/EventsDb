...
public class DatabaseConnectionHandler {
	...
	//aggregation max
	public int getMaxVolunteerHours() {
		int max = 0;
		try {
			Statement s = connection.createStatement();
			ResultSet rs = s.executeQuery("SELECT MAX(TimeVolunteered) as MaxVolunteerHours FROM Volunteer");
			while (rs.next()) {
				max = rs.getInt("MaxVolunteerHours");
			}
		} catch (SQLException e) {
			System.out.println(EXCEPTION_TAG + " " + e.getMessage());
			rollbackConnection();
	...
	//aggregation max
	public String getMaxVolunteerHourName(int maxVal){
		String maxName = "";
		try {
			Statement s = connection.createStatement();
			ResultSet rs = s.executeQuery("SELECT Username as TopVolunteer FROM Volunteer WHERE TimeVolunteered = " + String.valueOf(maxVal));
			while (rs.next()) {
				maxName = rs.getString("TopVolunteer");
			}
		} catch (SQLException e) {
			System.out.println(EXCEPTION_TAG + " " + e.getMessage());
			rollbackConnection();
	...
	//nested aggregation
	// public void getVolunteerCountPerEvent(){
	// 	try {
	// 		Statement s = connection.createStatement();
